<div id="panel">

	<div id = "intro">
		<div id="titreforumintro">Bienvenue sur le forum de Tune in Town </div></br>
		<p>Vous trouverez ici différentes rubriques pour exprimer vos opinions, </br> 
			débattre sur les différents artistes et accéder à la F.A.Q. </br>
			Exprimez vos opinions ...</p>
		<div id="charte">
			<a href="index.php?page=charteForumcontrolleur">Charte du forum</a>
		</div>		
	</div>


	<div id="sujet">
		<ul>
			<li class="soussujet"> <h1>Discussion</h1>
					<ul>		
						<li><a href="index.php?page=liste_topiccontrolleur&categorie=General"><h2>Général</h2></a></li>
						<p>Dans ce forum parlez de tout et de <br/>
							rien bref de ce que vous voulez.</p>
						
						<li><a href="index.php?page=liste_topiccontrolleur&categorie=Style_de_musique"><h2>Styles de musique</h2></a></li>
						<p>Ici vous pouvez discuter avec les autres <br/>
							membres de vos goûts musicaux. </p>
						<li><a href="index.php?page=liste_topiccontrolleur&categorie=Salles"><h2>Salles</h2></a></li>	
						<p>Vous êtes déjà allé dans cette salle ? <br/>
						Vous voulez en parler ? c'est ici. </p>
					</ul>
			</li>
			<li class="sousujet"> <h1>Promotions</h1>
					<ul>
						<li><a href="index.php?page=liste_topiccontrolleur&categorie=Artiste"><h2>Artiste</h2></a></li>
						<p>Pour parler de ses artistes préférés.</p>
						<li><a href="index.php?page=liste_topiccontrolleur&categorie=Salle"><h2>Salle</h2></a></li>

						<li><a href="index.php?page=liste_topiccontrolleur&categorie=Concert"><h2>Concert</h2></a></li>
					</ul>
			</li>
			<li class="soussujet"> <h1>F.A.Q</h1>

			</li>
		</ul>
	</div>	
</div>